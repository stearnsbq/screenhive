...
export class RoomComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
      ...
      this.peerConnection.addEventListener("datachannel", ({channel}) => {
        this.logging.info("data channel received")
        ...
        channel.addEventListener("open", ()=> {
          ...
          this.player.nativeElement.addEventListener("mousemove", (mouse) => {
            ...
            console.log(x, y)
            channel.send(JSON.stringify({event: "move-mouse", data: {x, y}}))
          })
		...
		this.route.params.subscribe(async ({ id }) => {
			...
			try {
				...
				const { isPrivate } = (await this.socketService.listenToEventOnce('is-room-private-success')) as {
					isPrivate: boolean;
				};
